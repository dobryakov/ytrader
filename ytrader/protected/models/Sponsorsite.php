...
class Sponsorsite extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('sponsor_id', 'length', 'max'=>20),
			array('name, join_url', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, sponsor_id, name, join_url', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'galleries' => array(self::HAS_MANY, 'Sponsorgallery', 'site_id'),
			'feeds' => array(self::HAS_MANY, 'Sponsorfeed', 'site_id'),
	        'galleriesCount' => array(self::STAT, 'Sponsorgallery', 'site_id'),
		);
	}
